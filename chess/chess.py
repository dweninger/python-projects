...
class ChessGame:
    def __init__(self, root):
        ...
        self.canvas = tk.Canvas(
            self.root, width=CANVAS_SIZE,
            height=CANVAS_SIZE, bg=DARK_BROWN)
        self.canvas.pack()

        self.canvas.bind(
            "<Button-1>", self.on_square_click)
    ...
    def draw_board(self):
        ...
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                ...
                color = LIGHT_BROWN if (
                    (row + col) % 2 == 0) else DARK_BROWN
        ...
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                ...
                if (piece.lower() == 'k') and (
                    piece.islower() == (player == 'black')):
                    king_position = (row, col)

        if king_position is None:
            return False  # Player's king not found

        # Loop through all opponent's pieces to see 
        # if any can capture the king
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                piece = board[row][col]
                if (piece.lower() != 'k') and (
    ...
    def is_checkmate(self, player):
        ...
        player_pieces = [
            piece for row in self.board 
            for piece in row if 
            (player == 'black' and piece.islower()) 
            or (player == 'white' and piece.isupper())
        ]
        # Iterate through player's pieces 
            ...
            for col1 in range(BOARD_SIZE):
                ...
                if piece in player_pieces:
                    for row2 in range(BOARD_SIZE):
                        for col2 in range(BOARD_SIZE):
                            if self.is_valid_move(
                                (row1, col1), (row2, col2), self.board):
                                # Try making move on a copy of 
                                # board to check for king in check
                                board_copy = [
                                    row[:] for row in self.board]
                                board_copy[row2][col2] = board_copy[
                                    row1][col1]
        ...
        # No valid move to get out of check
        return True
    ...
    def on_square_click(self, event):
        ...
        if self.selected_square is None:
            ...
            if piece != ' ' and (
                (self.current_player == 'white' 
                 and piece.isupper()) 
                 or (self.current_player == 'black' 
                     and piece.islower())):
                self.selected_square = square
                self.highlight_selected_square(square)
        else:
            if square == self.selected_square:
                # Clicking same piece again unselects it
                self.selected_square = None
                self.remove_highlight()
            elif self.is_valid_move(
                self.selected_square, square, self.board):
                # Try making move on a copy 
                # of board to check for king in check
                board_copy = [row[:] for row in self.board]
                board_copy[row][col] = board_copy[
                    self.selected_square[0]][self.selected_square[1]]
                board_copy[self.selected_square[0]][
                    self.selected_square[1]] = ' '

                # After making move, check if king is still in check
                if not self.is_king_in_check(
                    self.current_player, board_copy):
                    ...
                    if winner is not None and winner != '':
                        if winner == 'stalemate':
                            ...
                            self.check_label.config(
                                text=f"Checkmate! {winner.capitalize()}"
                                " wins!")
                else:
                    self.check_label.config(
                        text="Invalid move! Your king is in check.")
            else:
                # Clicking on a different piece with an invalid 
                # move reselects the new piece
                if piece != ' ' and (
                    (self.current_player == 'white' 
                        and piece.isupper()) or 
    ...
    def highlight_selected_square(self, square):
        ...
        self.selected_square_highlight = self.canvas.create_rectangle(
            x0, y0, x1, y1, outline="light blue", 
    ...
    def is_valid_move(self, from_square, to_square, board):
        ...
        if piece == 'p' or piece == 'P':
            return self.is_valid_move_pawn(
                from_square, to_square, board)

        if piece == 'r' or piece == 'R':
            return self.is_valid_move_rook(
                from_square, to_square, board)

        if piece == 'n' or piece == 'N':
            return self.is_valid_move_knight(
                from_square, to_square, board)

        if piece == 'b' or piece == 'B':
            return self.is_valid_move_bishop(
                from_square, to_square, board)

        if piece == 'q' or piece == 'Q':
            return self.is_valid_move_queen(
                from_square, to_square, board)

        if piece == 'k' or piece == 'K':
            return self.is_valid_move_king(
                from_square, to_square, board)
    ...
    def is_valid_move_pawn(self, from_square, to_square, board):
        ...
        if (col1 == col2 and 
            row2 == row1 + direction and 
            board[row2][col2] == ' '):
            return True

        # Initial double pawn move (two squares forward)
        if (col1 == col2 and 
            row2 == row1 + 2 * direction and 
            row1 in (1, 6) and 
            board[row2][col2] == ' '):
            return True
        ...
        if abs(col2 - col1) == 1 and row2 == row1 + direction:
            ...
            if (target_piece != ' ' and 
                target_piece.islower() != piece.islower()):
                return True

        # En passant capture
        if (abs(col2 - col1) == 1 and 
            row2 == row1 + direction and 
            (row1 + direction, col2) == self.last_double_move_pawn):
            return True
        ...
        if row1 == row2:
            if col1 < col2:
                for col in range(col1 + 1, col2):
                    if board[row1][col] != ' ':
                        return False
                return (board[row2][col2] == ' ' or 
                        board[row2][col2].islower() != piece.islower())
    ...
    def is_valid_move_bishop(self, from_square, to_square, board):
        ...
        # Check if the bishop is moving through or 
        # landing on its own pieces
    ...
    def is_valid_move_knight(self, from_square, to_square, board):
        ...
        if (delta_row == 1 and delta_col == 2) or (
    ...
    def is_valid_king_side_castling(self):
        ...
        if (self.is_square_under_attack((7, 4)) or 
            self.is_square_under_attack((7, 5)) or 
            self.is_square_under_attack((7, 6))):
            return False
    ...
    def is_square_under_attack(self, square):
        ...
        piece = self.board[row][col]
        opponent = ('black' 
            if self.current_player == 'white' else 'white')

        # Loop through all opponent's pieces 
        # to see if any can capture the square
        for row2 in range(BOARD_SIZE):
            for col2 in range(BOARD_SIZE):
    ...
    def promote_pawn(self, row, col):
        ...
        for option in promotion_options:
            button = tk.Button(
                promotion_window, text=option, 
                command=lambda opt=option: promote_to(opt))
            button.pack()
    ...
    def move_piece(self, from_square, to_square):
        ...
        # Check for en passant capture and remove captured pawn
        if piece.lower() == 'p' and self.en_passant_capture:
            # En passant capture: Remove captured pawn
            self.board[row1][col2] = ' '
            self.en_passant_capture = False

        # Check if piece is a king and update movement history flags
        if piece.lower() == 'k':
        ...
        # Check if castling move and move rook accordingly
