...
def generate_word_search_grid(words_to_find):
    ...
    for word in words_to_find:
        ...
        while not placed:
            ...
            if all(
                grid[row + (
                    i if direction == 'vertical' 
                    else 0)][col + (
                        i if direction == 'horizontal' 
                        else 0)] == ' ' for i in range(
                            len(word))):
                placed = True
                if direction == 'horizontal':
...
class WordSearchGame:
    def __init__(self, root):
        ...
        self.canvas = tk.Canvas(
            self.root, width=360, height=400)
        self.canvas.grid(row=0, column=0)

        self.canvas.bind(
            "<ButtonPress-1>", self.on_start_drag)
        self.canvas.bind(
            "<B1-Motion>", self.on_drag)
        self.canvas.bind(
            "<ButtonRelease-1>", self.on_release_drag)

        self.selected_cells = []

        self.words_to_find = list(words_to_find)

        self.word_search_grid = generate_word_search_grid(
            self.words_to_find)

        self.create_grid()
        self.create_word_bank()
        self.create_start_over_button()
        ...
        for i in range(len(self.word_search_grid)):
            for j in range(len(self.word_search_grid[i])):
                ...
                self.canvas.create_text(
                    (x0 + x1) / 2, (y0 + y1) / 2, 
                    text=self.word_search_grid[i][j], 
                    font=("Helvetica", 12))
    ...
    def create_word_bank(self):
        ...
        self.word_bank_label = tk.Label(
            self.word_bank_frame, text="Word Bank:", 
            font=("Helvetica", 16))
        ...
        for word in self.words_to_find:
            label = Label(
                self.word_bank_frame, text=word, 
                font=("Helvetica", 14), padx=20, pady=0)
            label.pack()
    ...
    def draw_straight_line_selection(self, cell_size):
        for i in range(1, len(self.selected_cells)):
            x0 = self.selected_cells[i - 1][1] 
            x0 = x0 * cell_size + cell_size // 2
            y0 = self.selected_cells[i - 1][0] 
            y0 = y0 * cell_size + cell_size // 2
            x1 = self.selected_cells[i][1] 
            x1 = x1 * cell_size + cell_size // 2
            y1 = self.selected_cells[i][0] 
            y1 = y1 * cell_size + cell_size // 2

            selected_word = "".join(
                [self.word_search_grid[row][col] 
                 for row, col in self.selected_cells])

            if selected_word in self.words_to_find:
                self.canvas.create_line(
                    x0, y0, x1, y1, fill="blue", 
                    width=2, tags="foundword")
            else:
                self.canvas.create_line(
                    x0, y0, x1, y1, fill="blue", 
    ...
    def on_release_drag(self, event):
        ...
        for i, (row, col) in enumerate(
            self.selected_cells):
            if (0 <= row < len(self.word_search_grid) 
                and 0 <= col < len(
                    self.word_search_grid[0])):
                partial_word += (self.word_search_grid
                                 [row][col])

            if partial_word in self.words_to_find:
                matching_word_labels = [
                    label for label in self.word_labels 
                    if label.cget("text") == partial_word]
                    ...
                    for label in matching_word_labels:
                        label.config(
                            ...
                            font=(
                                "Helvetica", 14, 
                                'overstrike'))
                        self.words_to_find.remove(
                            partial_word)

        self.canvas.delete("selection")

        if not self.words_to_find:
            self.word_bank_label.config(
                text="Word Bank: All words found! You win!")
    ...
    def is_valid_selection(self, row, col):
        ...
        return (
            row == last_row 
            and abs(col - last_col) == 1) or (
                col == last_col 
                and abs(row - last_row) == 1)
